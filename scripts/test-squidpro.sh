...
if curl -s "$API_BASE/health" >/dev/null 2>&1; then
    ...
    echo "   Check if the service is running: docker compose -f docker/docker-compose.yml ps"
fi
...
echo "• View container status: docker compose -f docker/docker-compose.yml ps"
